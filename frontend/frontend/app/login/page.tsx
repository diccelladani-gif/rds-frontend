...
const ROLES = [
  { id: "admin"    as Role, icon: "⚙️", name: "Administrator", desc: "Full system access"    },
  { id: "reviewer" as Role, icon: "📋", name: "Reviewer",      desc: "Review & approve RDS"  },
  { id: "planner"  as Role, icon: "🏗️", name: "Planner",       desc: "Create & edit rooms"   },
  { id: "viewer"   as Role, icon: "👁️", name: "Viewer",        desc: "Read-only access"      },
];
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Save session
      sessionStorage.setItem("rds_user", JSON.stringify({
        name:  data.user.name,
        role:  data.user.role,
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brand-badge { display:inline-flex;align-items:center;gap:10px;background:rgba(37,99,235,0.15);border:1px solid rgba(37,99,235,0.3);border-radius:100px;padding:8px 18px;margin-bottom:48px;width:fit-content; }
        ...
        .logo-mark { width:44px;height:44px;border-radius:14px;background:linear-gradient(135deg,rgba(37,99,235,0.6),rgba(6,182,212,0.4));border:1px solid rgba(59,130,246,0.4);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:900;color:#93c5fd;box-shadow:0 8px 24px rgba(37,99,235,0.3); }
        ...
        .form-label { display:flex;align-items:center;gap:6px;font-size:11.5px;font-weight:700;color:rgba(255,255,255,0.55);letter-spacing:1px;text-transform:uppercase;margin-bottom:10px; }
        ...
        .form-input:focus { border-color:rgba(37,99,235,0.6);background:rgba(37,99,235,0.08);box-shadow:0 0 0 3px rgba(37,99,235,0.12); }
        .pass-input { padding-right:46px; }
        .eye-btn { position:absolute;right:14px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:rgba(255,255,255,0.3);font-size:16px;padding:4px;transition:color 0.2s; }
        ...
        .role-btn   { padding:10px 12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:11px;cursor:pointer;transition:all 0.2s;text-align:left; }
        ...
        .custom-check { width:18px;height:18px;border-radius:6px;border:1.5px solid rgba(255,255,255,0.13);background:transparent;display:flex;align-items:center;justify-content:center;transition:all 0.2s;flex-shrink:0;font-size:10px;color:#fff;font-weight:700; }
        ...
        .btn-login { width:100%;height:52px;background:linear-gradient(135deg,#2563eb,#1d4ed8);border:none;border-radius:14px;font-size:15px;font-weight:700;color:#fff;cursor:pointer;font-family:'Sora',sans-serif;position:relative;overflow:hidden;transition:all 0.25s;box-shadow:0 8px 24px rgba(37,99,235,0.35);display:flex;align-items:center;justify-content:center;gap:10px; }
        ...
        .error-box { font-size:12px;color:#f87171;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);border-radius:8px;padding:10px 14px;margin-bottom:18px;display:flex;align-items:center;gap:8px;animation:slideIn 0.3s ease both; }
        ...
        .sso-btn { width:100%;height:46px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:12px;display:flex;align-items:center;justify-content:center;gap:10px;font-size:13.5px;color:rgba(255,255,255,0.55);font-weight:600;cursor:pointer;transition:all 0.2s;font-family:'Sora',sans-serif; }
        ...
        .sec-badge { display:flex;align-items:center;gap:5px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:100px;padding:4px 10px; }
        ...
        .success-icon  { width:64px;height:64px;border-radius:50%;background:rgba(16,185,129,0.15);border:2px solid rgba(16,185,129,0.4);display:flex;align-items:center;justify-content:center;font-size:28px;box-shadow:0 0 40px rgba(16,185,129,0.2);animation:popIn 0.5s cubic-bezier(0.23,1,0.32,1) both; }
            ...
            {success ? (
              <div className="success-state">
                ...
                <div className="success-sub">
                  {successUser.name} · {ROLE_LABEL[successUser.role as Role] ?? successUser.role} — Redirecting…
                </div>
              </div>
            ) : (
              <>
